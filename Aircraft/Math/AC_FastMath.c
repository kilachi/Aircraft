...
void simple_3d_trans(float *ref, float *in, float *out)
{
	...
	h_tmp_x = fast_sqrtf(fast_pow(ref[2]) + fast_pow(ref[1]));
	h_tmp_y = fast_sqrtf(fast_pow(ref[2]) + fast_pow(ref[0]));
	
	pn = ref[2] < 0? -1 : 1;
	
	out[0] = ( h_tmp_x *in[0] - pn *ref[0] *in[2] ) ;
	out[1] = ( pn *h_tmp_y *in[1] - ref[1] *in[2] ) ;	
//	out[0] = h_tmp_x *in[0] - ref[0] *in[2];
//	out[1] = ref[2] *in[1] - ref[1] *in[2];
